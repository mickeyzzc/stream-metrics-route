...
flowchart TB
    ...
    subgraph 网关
        ...
        VALIDATE[验证<br/>大小限制]
        RELABEL[Relabel 过滤]
                ROUTE[路由器<br/>Jump Consistent Hash]
    end
...
实现带双重 Jump Consistent Hash 的路由逻辑：
...
- **Jump Consistent Hash 路由**：基于指标标签的一致性哈希
- **熔断器**：防止级联故障
- **重试逻辑**：对临时故障进行指数退避重试
...
双重 Jump Consistent Hash 算法解决确保相同指标发送到同一处理节点的问题：
...
// 第一次 Jump Hash：分配任务分区 ID
dime := common.JumpConsistentHash(uint64(hash), r.dimension)  // dimension 通常 = 100
...
### 为什么需要双重 Jump Consistent Hash？

| 场景 | 单一 Hash | 双重 Jump Hash |
|------|-------------|--------------|
